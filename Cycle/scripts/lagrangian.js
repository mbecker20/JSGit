...
class SplitLagrangian {
    constructor(funcs, params, pConst, damping, dx = .01) {
        ...
        // each func has properties paramKeys and paramDotKeys
        ...
        this.pDD = {};
        this.setParamKeys();
        this.setPartialDerivFuncs(funcs, dx);
        this.damping = damping;
    }
    ...
    setPartialDerivFuncs(dx) {
        this.pdfs = {standard: {}, dotted: {}} // partial deriv funcs
        for(var i = 0; i < this.paramKeys.length; i++) {
            var funcsWithParamKey = [];
            var funcsWithParamDotKey = [];
            for(var j = 0; j < this.funcs.length; j++) {
                for(var k = 0; k < this.funcs[j].paramKeys.length; k++) {
                    if(this.paramKeys[i] === this.funcs[j].paramKeys[k]) {
                        funcsWithParamKey.push(this.funcs[j]);
                    }
                }
                for(var k = 0; k < this.funcs[j].paramDotKeys.length; k++) {
                    if(this.paramDotKeys[i] === this.funcs[j].paramDotKeys[k]) {
                        funcsWithParamKey.push(this.funcs[j]);
                    }
                }
            }
            this.pdfs.standard[this.paramKeys[i]] = MF.MakePartialDerivFuncMult(funcsWithParamKey, this.paramKeys[i]);
            this.pdfs.dotted[this.paramDotKeys[i]] = MF.MakePartialDerivFuncMult(funcsWithParamDotKey, this.paramDotKeys[i]);
        }
    }
