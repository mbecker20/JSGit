...
function setUpUniCam(uniCam,canvas) {
    uniCam.inputs.clear();
    uniCam.inputs.addKeyboard();
    uniCam.inputs.addMouse();
    uniCam.inputs.addTouch();
    uniCam.keysUp=[32];
    uniCam.keysDown=[16];
    uniCam.speed=.5;
    uniCam.inputs.add(makeKBRotateInput(canvas));
    uniCam.attachControl(canvas, false);
    return uniCam;
}

function makeKBRotateInput2(canvas) {
    var kbRotateInput = function() {
        this._keys = [];
        this.keysLeft = [74];
        this.keysRight = [76];
